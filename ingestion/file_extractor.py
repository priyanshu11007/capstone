...
Schema enforcement:
  - All column types are cast explicitly to avoid INT64 vs STRING
    Parquet mismatches when Spark later reads across multiple files.
"""
import shutil
...
# ---------------------------------------------------------------------------
# Canonical dtypes for the UCI Online Retail schema.
# These are applied BEFORE writing to Parquet so every file has
# identical Parquet column types regardless of how pandas inferred them.
# ---------------------------------------------------------------------------
UCI_RETAIL_DTYPES: dict = {
    "InvoiceNo":   str,   # e.g. "536365" or "C536379" – must be string
    "StockCode":   str,   # e.g. "85123A"              – must be string
    "Description": str,
    "Quantity":    "Int64",   # nullable int (avoids float from NaN rows)
    "UnitPrice":   float,
    "CustomerID":  float,     # UCI stores as float; bronze cast to int later
    "Country":     str,
}
...
class FileExtractor(BaseExtractor):
    ...
    def extract(
        ...
        **kwargs,
        ...
        for fp in files:
            ...
            # ── SCHEMA ENFORCEMENT (prevents INT64 vs STRING mismatch) ──
            df = self._enforce_schema(df, entity)
    ...
    # Schema enforcement
    # ------------------------------------------------------------------
    def _enforce_schema(self, df: pd.DataFrame, entity: str) -> pd.DataFrame:
        """
        Apply canonical dtypes for known entities.
        For retail_sales: cast InvoiceNo, StockCode, Description → str so that
        every Parquet file has STRING columns, never INT64.
        Unknown entities pass through unchanged.
        """
        if entity != "retail_sales":
            return df

        df = df.copy()
        for col_name, dtype in UCI_RETAIL_DTYPES.items():
            if col_name not in df.columns:
                continue
            try:
                if dtype == str:
                    # fillna("") before astype so NaN doesn't become "nan"
                ...
                self.logger.warning(f"Could not cast {col_name} to {dtype}: {e}")

        self.logger.info("Schema enforcement applied (retail_sales).")
        return df
    ...
    def _read_file(self, fp: Path) -> pd.DataFrame:
        ext = fp.suffix.lower()
        if ext == ".csv":
            # dtype=str on read prevents pandas from guessing INT for InvoiceNo
